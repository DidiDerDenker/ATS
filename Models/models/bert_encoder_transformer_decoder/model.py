...
train_dataset = file_based_input_fn_builder(
    input_file=c.train_out_file,
    max_seq_length_src=c.max_seq_length_src,
    max_seq_length_tgt=c.max_seq_length_tgt,
    is_training=True,
    drop_remainder=True,
    is_distributed=c.is_distributed
)({"batch_size": c.batch_size})

eval_dataset = file_based_input_fn_builder(
    input_file=c.eval_out_file,
    max_seq_length_src=c.max_seq_length_src,
    max_seq_length_tgt=c.max_seq_length_tgt,
    is_training=True,
    drop_remainder=True,
...
bert_config = model_utils.transform_bert_to_texar_config(os.path.join(c.bert_pretrain_dir, "bert_config.json"))

tokenizer = tokenization.FullTokenizer(
        vocab_file=os.path.join(c.bert_pretrain_dir, "vocab.txt"),
        do_lower_case=True)
...
iterator = tx.data.FeedableDataIterator({"train": train_dataset, "eval": eval_dataset})
...
with tf.variable_scope("bert"):
        embedder = tx.modules.WordEmbedder(vocab_size=bert_config.vocab_size, hparams=bert_config.embed)
        ...
        encoder = tx.modules.TransformerEncoder(None) # TODO: Use bert_config.encoder for hyperparameters
...
init_checkpoint = os.path.join(c.bert_pretrain_dir, "bert_model.ckpt")
...
decoder = tx.modules.TransformerDecoder(embedding=tgt_embedding, hparams=c.decoder_config)
...
mle_loss = transformer_utils.smoothing_cross_entropy(outputs.logits, labels, vocab_size, c.loss_label_confidence)
...
start_tokens = tf.fill([tx.utils.get_batch_size(src_input_ids)], c.bos_token_id)
