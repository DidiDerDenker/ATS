...
class DirectoryProcessor:
    ...
    def process(input_dir, output_dir, function):
        """ Apply function to all files in input_dir and save the resulting output files in output_dir. """
...
class Rouge155(object):
    ...
    @property
    def bin_path(self):
        """ The full path of the ROUGE binary, i.e. rouge_home_dir/ROUGE-1.5.5.pl. """
        if self._bin_path is None:
            raise Exception(
                "ROUGE path not set. Please set the ROUGE home directory and ensure that ROUGE-1.5.5.pl exists in it.")
    ...
    def convert_text_to_rouge_format(text, title="dummy title"):
        ...
        sent_elems = ["<a name=\"{i}\">[{i}]</a> <a href=\"#{i}\" id={i}>{text}</a>".format(i=i, text=sent)
    ...
    def write_config_static(system_dir, system_filename_pattern,
        ...
        if not system_models_tuples:
            raise Exception("Did not find any files matching the pattern {} in the system summaries directory {}."
                .format(system_filename_pattern.pattern, system_dir))
    ...
    def output_to_dict(self, output):
        """ Convert the ROUGE output into a python dictionary for further processing. """
    ...
    def __set_rouge_dir(self, home_dir=None):
        """ Verify presence of ROUGE-1.5.5.pl, data folder and set those paths. """
