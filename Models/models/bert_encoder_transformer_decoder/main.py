...
def _eval_epoch(sess, epoch, mode):
    ...
    fetches = {"inferred_ids": inferred_ids, }
    bno = 0
    ...
    if mode == "eval":
        fname = os.path.join(model_dir, "tmp.eval")  # TODO: Use ROUGE, not BLEU

        hypotheses = tx.utils.str_join(hypotheses)
        references = tx.utils.str_join(references)
        hyp_fn, ref_fn = tx.utils.write_paired_text(hypotheses, references, fname, mode="s")
