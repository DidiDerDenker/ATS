...
def file_based_convert_examples_to_features(examples, max_seq_length_src, max_seq_length_tgt, tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        if (ex_index+1) % 1000 == 0:
            print("Processed {} examples".format(ex_index)) # TODO: Remove
...
def convert_single_example(ex_index, example, max_seq_length_src, max_seq_length_tgt, tokenizer):
    ...
    feature = InputFeatures(
        src_input_ids=input_ids_src,
        src_input_mask=input_mask_src,
        src_segment_ids=segment_ids_src,
        tgt_input_ids=input_ids_tgt,
        tgt_input_mask=input_mask_tgt,
...
def get_dataset(processor,
    ...
    """
    Args:
        processor: Data Preprocessor, must have get_labels, get_train/dev/test/examples methods defined.
...
def main():
    ...
    # TODO: Prepare dataset as described in the readme and place it in the data-directory

    train_dataset = get_dataset(processor, tokenizer, c.data_dir,
                                c.max_seq_length_src, c.max_seq_length_tgt,
                                c.batch_size, "train", c.data_dir)

    eval_dataset = get_dataset(processor, tokenizer, c.data_dir,
                               c.max_seq_length_src, c.max_seq_length_tgt,
                               c.eval_batch_size, "eval", c.data_dir)
