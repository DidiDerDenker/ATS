...
class Optimizer(object):
    ...
    or https://keras.io/optimizers/.

    Recently there are slightly different values used in the paper
    "Attention is all you need" in https://arxiv.org/pdf/1706.03762.pdf,
    particularly the value beta2=0.98 was used there however,
    beta2=0.999 is still arguably the more established value,
    so we use that here as well.
    """

    def __init__(self, method, learning_rate, max_grad_norm, lr_decay=1,
                 beta1=0.9, beta2=0.999, adagrad_accum=0.0, warmup_steps=4000, weight_decay=0,
                 start_decay_steps=None, decay_steps=None, decay_method=None):
        self.last_ppl = None
    ...
    def set_parameters(self, params):
        ...
        if self.method == "sgd":
            ...
            raise RuntimeError("Invalid optimization method: " + self.method)
    ...
    def step(self):
        """ Update the model parameters based on current gradients, optionally update learning rate. """

        self._step += 1

        if self.decay_method == "noam":
            self._set_rate(self.original_lr * min(self._step ** (-0.5), self._step * self.warmup_steps**(-1.5)))
