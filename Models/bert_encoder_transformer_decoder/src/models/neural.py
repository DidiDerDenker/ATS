...
class GlobalAttention(nn.Module):
    """
    Global attention module, following the paper of Luong and Bahdanau
    that takes a matrix and a query vector. It then computes a parameterized
    convex combination of the matrix based on the input query.

    Constructs a unit mapping a query q of size dim
    and a source matrix H of size n x dim, to an output
    of size dim.

    All models compute the output as math:c = sum_{j=1}^{SeqLength} a_j H_j
    where math:a_j is the softmax of a score function.
    Then they apply a projection layer to [q, c].
    ...
    * math:score(H_j, q) = v_a^T tanh(W_a q + U_a h_j)
    ...
    def score(self, h_t, h_s):
        ...
        Returns:
          obj:FloatTensor: raw attention scores (unnormalized) for each src index [batch x tgt_len x src_len]
        """
...
class MultiHeadedAttention(nn.Module):
    ...
    Similar to standard dot-attention but uses multiple attention
    distributions simultaneously to select relevant items.
    ...
    def forward(self, key, value, query,
                mask=None, layer_cache=None, type=None, predefined_graph_1=None):
        """
...
class DecoderState(object):
    ...
    input-feeding and non-recurrent models. Modules need to implement
    this to utilize beam search decoding.
    """
