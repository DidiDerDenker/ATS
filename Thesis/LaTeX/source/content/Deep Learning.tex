...
\begin{figure}[h!]
  \centering
  \fbox{\includegraphics[width=0.5\linewidth]{./source/images/multiheadattention.png}}
...
\noindent
Architektonisch werden die rekurrenten Schichten bisheriger Sequence-to-Sequence-Modelle durch Transformer-Module ersetzt. Diese bestehen aus einer Multi-Head-Attention-Schicht, einem positionsabhängigen Feed-Forward-Netzwerk und einer sogenannten Layer-Normalization-Schicht. Letztere werden nun in Bezug auf Transformer intensiver erläutert \cite[S.~399]{ZHA20}. \autoref{pic:TransformerArchitecture} visualisiert die Architektur im Vergleich zu \autoref{pic:AttentionMechanism}.\\

\begin{figure}[h!]
  \centering
  \fbox{\includegraphics[width=0.5\linewidth]{./source/images/transformerarchitecture.png}}
  \caption{Transformer-Architektur \cite[S.~399]{ZHA20}.}
  \label{pic:TransformerArchitecture}
\end{figure}

\noindent
Feed-Forward-Netzwerk (positionsabhängig), Absatz ab S. 402 in ZHA20 verschriften.\\

\noindent
Layer-Normalization-Schicht: Diese wird benötigt, um für das entsprechende Modell einen generalisierenden Effekt zu erzielen. Dabei werden keine neuen Abhängigkeiten erlernt, wohl aber die Trainingszeit weiter reduziert. -> Weiteres siehe ab S. 403 in ZHA20, auch S. 404 bzgl. Positional Encoding irgendwo unterbringen.\\

\noindent
Unterschiede im Encoder und Decoder mit S. 406 ff. in ZHA20 verschriften.\\
...
-> PDF generieren und Kapitel optisch aufhübschen
