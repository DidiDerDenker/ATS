...
\noindent
Bislang wissenschaftlich nicht im Kontext der \ac{ATS} erprobt und daher als selbst konstruierter Ansatz zu bewerten ist die Nutzung sogenannter  Sentence-Transformer anstatt von \ac{ROUGE} oder \ac{BLEU}. Letztere sind nicht in der Lage, zwei Texte wie die \ac{REFZ} und die \ac{SYSZ} semantisch miteinander zu vergleichen. Prinzipiell betten Sentence-Transformer multilinguale Texte auf Satzebene in einen Vektorraum ein. Anschließend ist die Distanz der Vektoren zweier eingehender Sätze messbar. Möglich ist es demnach, die generierten Zusammenfassungen im Trainingsprozess mit den Distanzen der Sentence-Transformer zu bewerten und entsprechend zu optimieren \cite{FEN20}.\\
